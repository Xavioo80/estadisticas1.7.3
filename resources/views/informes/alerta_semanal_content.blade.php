...
<div class="informe-filters-card mb-3 no-print">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
        <form id="filter-form" action="{{ route('informes.alerta-semanal') }}" method="GET" class="d-flex flex-wrap align-items-center gap-3">
            ...
            <div class="filter-group">
                <label class="filter-label">Semana Epidemiológica</label>
                <select name="se" class="form-control form-control-sm ajax-filter" style="min-width: 160px;">
        ...
        <div class="d-flex align-items-center gap-2">
            <div class="text-right mr-3 d-none d-sm-block">
                <div style="font-size: 0.75rem; color: var(--text-muted); font-weight: 500;">Rango Epidemiológico</div>
                <div style="font-size: 0.88rem; font-weight: 700; color: var(--color-primary);">{{ $fechaInfo['start'] }} al {{ $fechaInfo['end'] }}</div>
            </div>
...
<div class="informe-table-container custom-scrollbar mb-4" style="background: var(--bg-surface); border: 1px solid var(--border-color); border-radius: var(--radius-md); overflow: auto; box-shadow: var(--shadow-sm);">
    <table class="table table-bordered table-alerta mb-0" style="width: 100%; border-collapse: separate; border-spacing: 0;">
        <thead>
            <tr style="background: var(--bg-subtle);">
                <th rowspan="2" class="sticky-header text-left" style="width: 38%; padding: 10px 14px; font-size: 0.82rem; font-weight: 700; color: var(--text-primary); border-color: var(--border-color); vertical-align: middle;">
                    ENFERMEDADES / EVENTOS
                </th>
                <th colspan="4" class="sticky-header text-center" style="padding: 8px 10px; font-size: 0.8rem; font-weight: 700; color: var(--text-primary); border-color: var(--border-color);">
                    NÚMERO DE CASOS POR GRUPO DE EDAD
                </th>
                <th rowspan="2" class="sticky-header text-center" style="width: 10%; padding: 10px 14px; font-size: 0.82rem; font-weight: 700; color: var(--text-primary); border-color: var(--border-color); vertical-align: middle;">
                    TOTAL
                </th>
            </tr>
            <tr style="background: var(--bg-subtle);">
                <th class="sticky-header text-center" style="width: 13%; padding: 6px 10px; font-size: 0.76rem; font-weight: 600; color: var(--text-muted); border-color: var(--border-color);">&lt; 1 AÑO</th>
                <th class="sticky-header text-center" style="width: 13%; padding: 6px 10px; font-size: 0.76rem; font-weight: 600; color: var(--text-muted); border-color: var(--border-color);">1 - 4 AÑOS</th>
                <th class="sticky-header text-center" style="width: 13%; padding: 6px 10px; font-size: 0.76rem; font-weight: 600; color: var(--text-muted); border-color: var(--border-color);">5 - 14 AÑOS</th>
                <th class="sticky-header text-center" style="width: 13%; padding: 6px 10px; font-size: 0.76rem; font-weight: 600; color: var(--text-muted); border-color: var(--border-color);">15 Y + AÑOS</th>
            </tr>
        </thead>
        <tbody>
            @foreach($rowsDef as $idx => $row)
                <tr style="border-color: var(--border-color);">
                    <td class="text-left font-weight-600" style="padding: 8px 14px; color: var(--text-primary); font-size: 0.85rem; border-color: var(--border-color);">
                        {{ $row['label'] }}
                    </td>

                    @if($row['label'] === 'CADENA RED DE FRIO')
                        <td colspan="4" class="text-center cursor-pointer font-weight-bold select-none p-1" id="coldChainCell" onclick="toggleColdChain()" style="background: #22c55e; color: #fff; font-size: 0.85rem; border-color: var(--border-color);">
                            <span id="coldChainLabel">VERDE</span>
                        </td>
                        <td class="text-center text-muted" style="padding: 8px 10px; border-color: var(--border-color);">-</td>
                    @else
                        <!-- < 1 AÑO -->
                        <td class="text-center cell-clickable {{ $results[$idx]['less_1'] > 0 ? 'font-weight-bold text-primary' : 'text-muted' }}" 
                            ...
                            style="padding: 8px 10px; font-size: 0.95rem; cursor: pointer; border-color: var(--border-color); transition: background 0.15s;">
                        ...
                        <td class="text-center cell-clickable {{ $results[$idx]['total'] > 0 ? 'font-weight-bold' : 'text-muted' }}" 
                            ...
                            style="padding: 8px 10px; font-size: 0.95rem; cursor: pointer; background: var(--bg-subtle); color: var(--text-primary); border-color: var(--border-color); transition: background 0.15s;">
                            {{ $results[$idx]['total'] ?: '0' }}
                        </td>
                    @endif
                </tr>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content" style="background: var(--bg-surface); border: 1px solid var(--border-color); color: var(--text-primary); border-radius: var(--radius-md); box-shadow: var(--shadow-xl);">
            <div class="modal-header" style="border-bottom-color: var(--border-color);">
                <div>
                    <h5 class="modal-title font-weight-bold" id="modalAlertaTitle" style="color: var(--text-primary);"></h5>
                    <div class="d-flex align-items-center gap-2 mt-1">
                        <span class="badge badge-primary" id="modalAlertaRangeLabel" style="font-size: 0.72rem;"></span>
